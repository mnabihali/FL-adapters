...
def main():
    ...
    if args.dataset.lower() == "voxpopuli":

        voxpopuli = load_dataset("facebook/voxpopuli", "en", cache_dir="/stek/mohamed/corpous")
        voxpopuli = voxpopuli.remove_columns(
            ["language", "raw_text", "gender", "speaker_id", "is_gold_transcript", "accent"])
        ...
        vocabs = voxpopuli.map(extract_all_chars, batched=True, batch_size=-1, keep_in_memory=True,
                               remove_columns=voxpopuli.column_names["train"])
        ...
        librispeech_train = load_dataset('librispeech_asr', 'clean', split='train.100',
                                         cache_dir='/stek/mohamed/corpous')
        librispeech_dev = load_dataset('librispeech_asr', 'clean', split='validation',
                                       cache_dir='/stek/mohamed/corpous')
        librispeech_test = load_dataset('librispeech_asr', 'clean', split='test', cache_dir='/stek/mohamed/corpous')
        ...
        vocabs_train = librispeech_train.map(extract_all_chars, batched=True, batch_size=-1, keep_in_memory=True,
    ...
    if args.dataset.lower() in ["librispeech", "voxpopuli"]:
        # processor
    ...
    # config
    config = Wav2Vec2Config.from_pretrained("facebook/wav2vec2-large-100k-voxpopuli",
    ...
    model = Wav2Vec2ForCTC.from_pretrained("facebook/wav2vec2-large-100k-voxpopuli", config=config,
